"""Tests for mHC-LoRA-Residual-prenorm (RMSNorm on h, before the LoRA down projection).

Run from the repo root (mhc-lite) with:
    python -m hyper_conn.test_mhc_lora_residual_prenorm
"""
...
from .mhc_lora_residual_prenorm import (
    ManifoldConstrainedHyperConnectionsLoRAResidualPreNorm as MHCPre,
)
...
def _model(d=64, s=4, seed=0, cls=MHCPre):
...
def test_zero_B_keeps_init_identity():
    """B_s == 0 at init -> forward identical to midnorm (and to plain mHC)."""
    b, seq, d, s = 2, 5, 64, 4
    pre, mid = _model(d, s, cls=MHCPre), _model(d, s, cls=MHCMid)
    mid.load_state_dict(pre.state_dict())
...
def main():
    ...
    print("PASS: mhc_lora_residual_prenorm")
