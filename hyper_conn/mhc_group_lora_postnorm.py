...
"""
mHC-group-LoRA-postnorm  (ablation)
===================================

Norm-position ablation on the group-LoRA line: the parameter-free RMSNorm sits **after**
the LoRA up projection, i.e. it normalises the delta itself on the hidden dim:
...
The three positions under ablation on this line:
  * ``mhc_group_lora_prenorm``  : rmsnorm on the **hidden** dim of h, before A_s
  * ``mhc_group_lora_midnorm``  : rmsnorm on the **rank** dim, between A_s and B_s
  * ``mhc_group_lora_postnorm`` : rmsnorm on the **hidden** dim, after B_s   <-- this file

Relation to ``mhc_group_lora``: that module already computes this exact forward pass, so
this class only subclasses it and re-does the initialisation / weight-decay bookkeeping,
keeping the three ablation arms comparable:

  1. A_s init uses the corrected per-matrix fan_in (= d) via ``init_lora_A_per_stream_``.
     ``mhc_group_lora`` calls ``kaiming_uniform_`` on the 3D ``[s, d, r]`` tensor, which
     treats it as a conv weight (fan_in = d*r) and shrinks the init by sqrt(r).
  2. Both A_s and B_s are flagged ``_no_weight_decay``.  With the norm downstream of
     *both* matrices, delta depends only on their directions -- scaling either one is
     cancelled by the norm -- so decay would shrink ||A_s||, ||B_s|| without changing the
     forward pass, silently inflating their effective LR.  (midnorm flags A_s only, since
     there B_s is downstream of the norm and its scale is real.)

The group-wise H^pre read is untouched.  This mirrors ``mhc_lora_residual_postnorm`` on
the non-group line.
...
class ManifoldConstrainedHyperConnectionsGroupLoRAPostNorm(
    ManifoldConstrainedHyperConnectionsGroupLoRA
):
    """group-LoRA whose parameter-free RMSNorm is applied to delta after B_s."""
    ...
    # from ManifoldConstrainedHyperConnectionsGroupLoRA unchanged -- see module docstring.
...
def get_init_and_expand_reduce_stream_functions(
    ...
    **kwargs,
    ...
    hyper_conn_klass = (
        ManifoldConstrainedHyperConnectionsGroupLoRAPostNorm if not disable else Residual
    )
...
MHCGroupLoRAPostNorm = ManifoldConstrainedHyperConnectionsGroupLoRAPostNorm
