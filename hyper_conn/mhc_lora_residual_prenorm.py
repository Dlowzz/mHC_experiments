...
"""
mHC-LoRA-Residual-prenorm  (ablation)
=====================================

Norm-position ablation: the parameter-free RMSNorm sits **before** the LoRA down
projection, i.e. it normalises the branch output h itself (hidden dim), not the rank
activations and not the delta:
...
Contrast with the other two positions:
  * ``mhc_lora_residual_midnorm``  : rmsnorm on the **rank** dim, between A_s and B_s
  * ``mhc_lora_residual_postnorm`` : rmsnorm on the **hidden** dim, after B_s
...
from .mhc_lora_residual import rmsnorm_lastdim
from .mhc_lora_residual_midnorm import (
    ManifoldConstrainedHyperConnectionsLoRAResidualMidNorm,
    init_lora_A_per_stream_,
)


class ManifoldConstrainedHyperConnectionsLoRAResidualPreNorm(
    ManifoldConstrainedHyperConnectionsLoRAResidualMidNorm
):
    """per-stream LoRA whose parameter-free RMSNorm is applied to h before A_s."""
...
def get_init_and_expand_reduce_stream_functions(
    ...
    **kwargs,
    ...
    hyper_conn_klass = (
        ManifoldConstrainedHyperConnectionsLoRAResidualPreNorm if not disable else Residual
    )
...
MHCLoRAResidualPreNorm = ManifoldConstrainedHyperConnectionsLoRAResidualPreNorm
