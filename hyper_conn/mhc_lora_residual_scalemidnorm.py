...
"""
mHC-LoRA-Residual-scale-midnorm  (ablation)
===========================================

``mhc_lora_residual_midnorm`` (RMSNorm on the LoRA **rank** dim between A_s and B_s)
plus a learnable **scale** on that norm -- and no bias:
...
at init this is exactly ``mhc_lora_residual_midnorm``; and since B_s is zero-init,
``delta_s == 0`` at init, so it also matches the original mHC.  It is flagged
...
delta and the rank-space beta-gated write.  The archived ``mhc_lora_residual_affinemidnorm``
overrode ``compute_lora`` only, which left the affine out of ``lora_write`` -- i.e. out of
the path ``depth_connection`` actually uses.  A unit test below pins this down.
...
class ManifoldConstrainedHyperConnectionsLoRAResidualScaleMidNorm(
    ManifoldConstrainedHyperConnectionsLoRAResidualMidNorm
):
    """midnorm LoRA with a learnable per-(stream, rank) scale on the rank-dim RMSNorm."""
...
def get_init_and_expand_reduce_stream_functions(
    ...
    **kwargs,
    ...
    hyper_conn_klass = (
        ManifoldConstrainedHyperConnectionsLoRAResidualScaleMidNorm if not disable else Residual
    )
...
MHCLoRAResidualScaleMidNorm = ManifoldConstrainedHyperConnectionsLoRAResidualScaleMidNorm
