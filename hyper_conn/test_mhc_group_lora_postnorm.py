"""Tests for mHC-group-LoRA-postnorm (RMSNorm on the hidden dim, after B_s).

Run from the repo root (mhc-lite) with:
    python -m hyper_conn.test_mhc_group_lora_postnorm
"""
...
from .mhc_group_lora import (
    ManifoldConstrainedHyperConnectionsGroupLoRA as MHCBase,
    rmsnorm_lastdim,
)
...
from .mhc_group_lora_postnorm import (
    ManifoldConstrainedHyperConnectionsGroupLoRAPostNorm as MHCPost,
)
...
def _model(d=64, s=4, seed=0, cls=MHCPost, **kw):
...
def test_group_read_untouched():
    ...
    d, s = 64, 4
    post, mid = _model(d, s, cls=MHCPost), _model(d, s, cls=MHCMid)
    assert torch.equal(post.group_pre_weight, mid.group_pre_weight)
    assert torch.equal(post.group_pre_bias, mid.group_pre_bias)
    x = _expand(torch.randn(2, 5, d), s)
    with torch.no_grad():
        gp, gm = post.get_group_gates(x), mid.get_group_gates(x)
    assert torch.equal(gp, gm)
    assert gp.shape[-2:] == (post.group_embedding_groups, s)
...
def test_optimizer_grouping():
    """The same id-based rule model.configure_optimizers uses must skip both matrices."""
    d, s = 64, 4
    m = _model(d, s)
    flagged = {id(p) for p in m.parameters() if getattr(p, "_no_weight_decay", False)}
    decay = {id(p) for p in m.parameters() if p.dim() >= 2 and id(p) not in flagged}
    assert id(m.stream_down_weight) not in decay
    assert id(m.stream_up_weight) not in decay
    assert id(m.group_pre_weight) in decay, "the group read weight should still decay"
...
def test_zero_B_keeps_init_identity():
    """B_s == 0 at init and rmsnorm(0) == 0 -> forward identical to midnorm."""
    b, seq, d, s = 2, 5, 64, 4
    post, mid = _model(d, s, cls=MHCPost), _model(d, s, cls=MHCMid)
...
def main():
    ...
    print("PASS: mhc_group_lora_postnorm")
