...
class GPT(nn.Module):
    ...
    def configure_optimizers(self, weight_decay, learning_rate, betas, device_type):
        ...
        # Exception: LoRA norm-affine params (lora_gamma / lora_bias) are norm parameters and
        # must NOT be weight-decayed even though they are 2D [streams, dim].
        no_decay_names = ("lora_gamma", "lora_bias")
        is_no_decay = lambda n, p: p.dim() < 2 or any(k in n for k in no_decay_names)
        decay_params = [p for n, p in param_dict.items() if not is_no_decay(n, p)]
        nodecay_params = [p for n, p in param_dict.items() if is_no_decay(n, p)]
        optim_groups = [
